...
const highlights = [
  ...
  },
  {
    ...
    description:
      "With hands-on experience in high-stakes events, we bring industrial-grade solutions to your organizer dashboard.",
    icon: FiCheckCircle,
  },
  {
      ...
      <div className="mx-auto max-w-full">
        ...
        <div className="section-container flex flex-col items-center text-center space-y-8 mb-24">
          <motion.p 
            ...
            className="text-[11px] font-bold uppercase tracking-[0.4em] text-orange"
          >
            Capabilities
          </motion.p>
          <motion.h2 
          ...
          <motion.p 
            ...
            className="max-w-2xl text-xl text-charcoal/40 font-medium leading-relaxed"
          >
          ...
          >
            {/* Edge Fades */}
            <div className="absolute left-0 top-0 z-10 h-full w-40 bg-linear-to-r from-cream to-transparent pointer-events-none" />
            <div className="absolute right-0 top-0 z-10 h-full w-40 bg-linear-to-l from-cream to-transparent pointer-events-none" />

            <div className="flex flex-col gap-4">
              <Marquee duration={50} pauseOnHover>
                {m1.map((item) => (
                  <Badge key={item} variant="outline" className="border-black/5 bg-white/40 px-6 py-3 rounded-none">
                    {item}
                  </Badge>
                ))}
              </Marquee>
        ...
        <div className="grid grid-cols-1 divide-y divide-black/5 divide-dashed border-t border-black/5 border-dashed sm:grid-cols-2 sm:divide-y-0 sm:divide-x lg:grid-cols-4 bg-white/20">
          {highlights.map((item, idx) => (
            <motion.div
              ...
              className="flex flex-col p-10 lg:p-14 group"
            >
              <div className="mb-12 flex h-14 w-14 items-center justify-center rounded-2xl bg-orange/5 text-orange transition-all duration-500 group-hover:bg-orange group-hover:text-white">
                <item.icon size={28} />
              </div>

              <div className="mt-auto space-y-4">
                <h3 className="font-display font-medium text-3xl tracking-tight text-charcoal">
                  {item.title}
                </h3>
                <p className="text-base text-charcoal/40 font-medium leading-relaxed">
                  {item.description}
                </p>
              </div>
            </motion.div>
